	...
	<xsl:template match="crush" xml:space="preserve">	
		...
		<xsl:variable name="cid" select="generate-id()" />
			...
			<td width="150" align="center" style="padding:0px 0px;">
				<span class="butspan" style="color:black;font-size:1.3em" onclick="populateTickerField('{ticker}');fill{$cid}();"><xsl:text>&#160;</xsl:text><xsl:value-of select="ticker" /><xsl:text>&#160;</xsl:text></span><br/>
				<br/>
				<!--
