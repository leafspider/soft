...
Grafico.Normaliser = Class.create({
  initialize: function(data, options) {
    ...
    // Data range
// jmh 2012-08-08    this.min = data.min();
// jmh 2012-08-08    this.max = data.max();
		if(isNaN(data[0]))
		{
			this.min = data.min();
	    this.max = data.max();
...
//		alert(min+","+max+","+range+","+d+","+precision+","+graphmin+","+graphmax);
...
//		alert(this.range+","+this.start_valued+","+this.zero_value);
  },
