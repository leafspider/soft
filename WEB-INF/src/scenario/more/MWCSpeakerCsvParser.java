...
public class MWCSpeakerCsvParser extends CsvParser 
{
...
//		Log.debug = true;
		try
		{
			File csvFile = new File( "C:\\Workspace\\Ultra\\Nick Trendov\\MWC\\0.Speaker.List.MWC.2015.csv" );
			File docFolder = new File( "C:\\Workspace\\Ultra\\Nick Trendov\\MWC\\speaker_pdf");			
			File textFolder = new File( docFolder.getParent() + "\\speaker_text" );
			String projectName = "MWC";
			
//			spiderUrls( csvFile.getAbsolutePath() );

//			extractText( docFolder, textFolder );
//			createDescriptions( csvFile.getAbsolutePath() );
			createEdges( projectName, csvFile, textFolder );
			createAlerts( textFolder.getParentFile() );
		}
		catch(Exception e)
		{
			e.printStackTrace();
...
//						URL feedUrl = new URL("http://127.0.0.1:8000/mwc/kube.post.xml?spinId=MWC\\" + booth + "&url=http://127.0.0.1:8080/soft/stand/mark/" + Util.replaceSubstring(name, " ", "%20") + "/feed.xml");
						URL feedUrl = new URL("http://127.0.0.1:8000/mwc/kube.post.xml?spinId=MWC\\" + hall + "&url=http://127.0.0.1:8080/soft/exhibitor/mark/" + Util.replaceSubstring(name, " ", "%20") + "/" + hall + "feed.xml");
...
//				if(n++ > 1) { break; }
	...
	public static void createAlerts(File outputFolder)
	{
		try
		{
			URL feedUrl = new URL("http://127.0.0.1:8000/mwc/alerts.post");
			File targetFolder = new File( outputFolder.getParentFile().getAbsolutePath() + "\\speaker_feed\\" );
			LinkContentDownloader dloader = new LinkContentDownloader( feedUrl, targetFolder );
			dloader.startThread();
			dloader.join();
		}
