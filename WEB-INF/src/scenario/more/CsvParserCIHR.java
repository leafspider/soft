...
public class CsvParserCIHR extends CsvParser 
{
...
//		Log.debug = true;
		try
		{
			File file = new File( "C:\\Workspace\\Ultra\\Ron\\CIHR\\Group C - Reviewer Profiles.csv" );
...
//			createEdges( file.getAbsolutePath() );
	...
	{
		File folder = new File( "C:\\Workspace\\Ultra\\Ron\\CIHR\\text" );
		folder.mkdirs();
		...
		try
		{
			...
			while(list.hasNext())
			{
				...
				{ 
					...
					{
						 text += vals[i].replaceAll(",", "\r\n\r\n");
					}
					text = text.replaceAll("\"", "");
					Log.infoln("" + name + " --- " + text); 

					File file = new File(folder.getAbsolutePath() + "\\" + name + ".txt");
					Util.writeAsFile(text, file.getAbsolutePath() );
				}
				n++;
//				if(n > 1) { break; }
			}
