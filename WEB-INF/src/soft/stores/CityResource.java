...
public class CityResource extends ObResource
{
	public static String resource = "city";
	public static String dispatch = "city";
...
//		Log.infoln( "CityResource.doGet()" );
		...
		{
			e.printStackTrace();
			Log.infoln( "CityResource.doGet status=404" );
			response.setStatus( 404 );
	...
	public synchronized void writeResponse( HttpServletRequest request, HttpServletResponse response ) throws Exception
    {
		...
		{
			Representation rep = CityRepresentation.dispatch( getRequest() );
    ...
    public String getListXml() throws ServletException, IOException, Exception
    {
    	...
    	Element root = new Element( "cityList" );
...
//   		root.addContent( new Element( "editor" ).addContent( "" + getRequest().isUserInRole( "editor" ) ) );
	    ...
	    Object count = session.createQuery( "select count(distinct city) from Deal" ).uniqueResult();
   		root.addContent( new Element( "count" ).addContent( new CDATA( "" + count ) ) );

	    Query query = session.createQuery( "select distinct city from Deal" );
    	...
    	while( records.hasNext() )
    	{
	    	...
	    	Element recordElem = new Element( "city" );
	    	recordElem.addContent( new Element( "city" ).addContent( new CDATA( "" + record ) ) );
