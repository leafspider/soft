...
public class StoreResource extends ObResource
{
	public static String resource = "store";
	public static String dispatch = "store";
    ...
    {
		Log.infoln( "StoreResource.doGet()" );
	...
	public synchronized void writeResponse( HttpServletRequest request, HttpServletResponse response ) throws Exception
    {
		...
		{
			Representation rep = StoreRepresentation.dispatch( getRequest() );
    ...
    public String getListXml() throws ServletException, IOException, Exception
    {
    	...
    	Element root = new Element( "storeList" );
...
//   		root.addContent( new Element( "editor" ).addContent( "" + getRequest().isUserInRole( "editor" ) ) );
	    ...
	    Object count = session.createQuery( "select count(distinct storeUrl) from Deal" ).uniqueResult();
   		root.addContent( new Element( "count" ).addContent( new CDATA( "" + count ) ) );

	    Query query = session.createQuery( "select distinct storeHost,city from Deal" );
