...
public class ProjectCCEMC extends Exhibitor 
{
	...
	{		
		try
		{
			...
			try { setName(doc.select( ".project_title" ).first().text()); } catch(Exception e1) {}	
			try 
			{ 
				String desc = "";
				Iterator els = doc.select( ".project_summary p, .project_summary div" ).iterator();
				while(els.hasNext())
				{
					Element el = (Element) els.next();
					String txt = el.ownText();//.replaceAll("Current Calls for Proposals", "");
					desc += txt + " ";
				}
				setDescription(desc);					
			} 
			catch(Exception e1) {}
//			Log.infoln( name + ": " + description );
